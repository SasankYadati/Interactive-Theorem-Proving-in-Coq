...
Definition filter_even_gt7 (l : list nat) : list nat := filter (fun x => (7 <? x) && (even x)) l. 
...
Theorem fold_length_lemma: forall X (l: list X) (x: X),
  fold_length (x :: l) = S (fold_length l).
Proof. 
  intros X. induction l.
  - reflexivity.
  - intros. rewrite -> IHl. unfold fold_length. simpl. reflexivity.
Qed.
...
Proof.
    intros X. induction l.
    - reflexivity.
    - try reflexivity. simpl. rewrite -> fold_length_lemma. f_equal. assumption.
Qed.
...
Definition fold_map {X Y: Type} (f: X -> Y) (l: list X) : list Y := fold (fun x v => (f x) :: v) l [].
...
Theorem fold_map_lemma: forall (X Y: Type) (f: X -> Y) (l: list X) (x: X),
    fold_map f (x::l) = f x :: fold_map f l.
Proof. 
    intros X Y f. induction l.
    - reflexivity.
    - intros. rewrite IHl. unfold fold_map. simpl. reflexivity.
Qed.
...
Proof.
    intros X Y f. induction l.
    - reflexivity.
    - simpl. rewrite <- IHl. apply fold_map_lemma.
Qed.
...
Definition mult (n m : cnat) : cnat := fun (X: Type) (f: X -> X) (x: X) => m X (fun (x: X) => n X f x) x.
